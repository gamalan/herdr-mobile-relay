...
Darwin)
	...
	restart)
		exec launchctl bootout "gui/$(id -u)/com.herdr-mobile-relay.service" 2>/dev/null
		exec launchctl bootstrap "gui/$(id -u)" "$HOME/Library/LaunchAgents/com.herdr-mobile-relay.service.plist"
		;;
	esac
	;;
Linux)
